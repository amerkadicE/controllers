...
class TokensController extends BaseController_1.BaseController {
    ...
    addToken(address, symbol, decimals, image) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const tokenMetadata = yield this.fetchTokenMetadata(address);
                const newEntry = {
                    address,
                    symbol,
                    decimals,
                    image,
                    isERC721,
                    aggregators: assetsUtil_1.formatAggregatorNames((tokenMetadata.aggregators || [])),
                };
     ...
     */
    addDetectedTokens(newDetectedTokens) {
        return __awaiter(this, void 0, void 0, function* () {
            const releaseLock = yield this.mutex.acquire();
            const { tokens, detectedTokens, ignoredTokens } = this.state;
            try {
                newDetectedTokens.forEach((tokenToAdd) => {
                    ...
                    const previousImportedEntry = tokens.find((token) => token.address.toLowerCase() === checksumAddress.toLowerCase());
                    if (previousImportedEntry) {
                        ...
                        if (ignoredTokenIndex === -1) {
                            // Add detected token
                            const previousDetectedEntry = detectedTokens.find((token) => token.address.toLowerCase() === checksumAddress.toLowerCase());
                            if (previousDetectedEntry) {
                                const previousDetectedIndex = detectedTokens.indexOf(previousDetectedEntry);
    ...
    removeAndIgnoreToken(address) {
        ...
        const newTokens = tokens.filter((token) => token.address.toLowerCase() !== address.toLowerCase());
        const newDetectedTokens = detectedTokens.filter((token) => token.address.toLowerCase() !== address.toLowerCase());
    ...
    fetchTokenMetadata(tokenAddress) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const token = yield token_service_1.fetchTokenMetadata(ethereumjs_util_1.isHexString(this.config.chainId)
                    ? util_1.convertPriceToDecimal(this.config.chainId).toString()
                    : this.config.chainId, tokenAddress, this.abortController.signal);
                return token;
            }
            catch (_a) {
                return {};
